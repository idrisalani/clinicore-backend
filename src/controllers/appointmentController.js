...
const appointmentSchema = Joi.object({
  ...
  reason_for_visit: Joi.string().required().messages({
...
export const createAppointment = async (req, res) => {
  try {
    ...
    // Check for conflicts (same patient, overlapping time)
...
export const getPatientAppointments = async (req, res) => {
  try {
    ...
    if (upcoming) {
      whereClause += ' AND a.appointment_date >= date("now")';
    }
...
export const getDoctorAvailability = async (req, res) => {
  try {
    ...
    // Define available time slots (9 AM to 5 PM, 30 min slots)
...
export const getAppointmentStats = async (req, res) => {
  try {
    ...
    // Total appointments
    const totalResult = await query('SELECT COUNT(*) as total FROM appointments WHERE status != "Cancelled"');
