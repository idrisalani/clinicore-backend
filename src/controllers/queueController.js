...
export const getTodayQueue = async (req, res) => {
  try {
    ...
    const countRow = await getOne(
      `SELECT COUNT(*) AS total FROM queue q ${whereClause}`,
      params
    );
...
// ── GET /queue/next — next patient to call ────────────────────────────────────
export const getNextPatient = async (req, res) => {
  try {
    const doctorId = req.query.doctor_id || '';
    let sql = `
      SELECT q.*, p.first_name, p.last_name, p.phone, p.blood_type, p.allergies
      FROM queue q
      JOIN patients p ON q.patient_id = p.patient_id
...
// ── POST /queue/check-in — add patient to queue ───────────────────────────────
export const checkInPatient = async (req, res) => {
  try {
    const {
      patient_id, appointment_id, doctor_id,
      priority = 'Normal', reason_for_visit, notes,
    } = req.body;
    ...
    const existing = await getOne(
      `SELECT queue_id, status FROM queue
       WHERE patient_id = ? AND queue_date = ?
         AND status NOT IN ('Completed','No-Show','Skipped')`,
      [patient_id, date]
    );
    if (existing) {
    ...
    const lastNum = await getOne(
      'SELECT MAX(queue_number) AS last_num FROM queue WHERE queue_date = ?',
      [date]
    );
    ...
    const newEntry = await getOne(`
      ...
      FROM queue q
      JOIN patients p ON q.patient_id = p.patient_id
      WHERE q.queue_date = ? AND q.patient_id = ?
        AND q.queue_number = ?
...
export const updateQueueStatus = async (req, res) => {
  try {
    ...
    // Set time stamps based on status transition
    ...
    if (status === 'Called' && !callTime) {
      callTime = nowStr;
      waitMins = Math.round(
        (new Date(nowStr) - new Date(entry.check_in_time)) / 60000
      );
    }
    ...
    await query(`
      UPDATE queue SET
        ...
        served_by   = CASE WHEN ? IN ('Completed','No-Show','Skipped')
                           THEN ? ELSE served_by END,
        updated_at  = ?
      WHERE queue_id = ?
    `, [
...
// ── PUT /queue/:id — update queue details ─────────────────────────────────────
...
// ── GET /queue/patient/:patientId — patient queue history ─────────────────────
