...
// which works on Render, Vercel, Lambda etc.
//
// Install:
...
const CLINIC_RC      = process.env.CLINIC_RC      || '';   // RC number if incorporated
    ...
    <div style="padding:24px">
        ...
        <div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <div>
              ...
              <div class="info-value">${fmtDate(invoice.invoice_date)}</div>
            </div>
            <div>
              <div class="info-label">Due Date</div>
              <div class="info-value ${isOverdue ? 'red' : ''}">${fmtDate(invoice.due_date)}</div>
            </div>
      ...
      <div class="amount-box">
        <div>
          ...
          <div class="amount-due-val ${isPaid ? 'green' : isOverdue ? 'red' : 'teal'}">
      ...
      <table>
        <thead>
          <tr>
            <th>Description</th><th>Code</th><th style="text-align:center">Qty</th>
      ...
      <div style="background:#F8FAFC;border-radius:6px;padding:12px;font-size:11px;color:#475569">${invoice.notes}</div>` : ''}
...
export const generatePatientSummaryPDF = async ({
  ...
  const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"><style>${baseCSS}
  .patient-avatar { width:60px;height:60px;border-radius:50%;background:rgba(255,255,255,0.2);
    display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:white;
    flex-shrink:0;line-height:60px;text-align:center; }
  ...
  <div style="max-width:720px;margin:0 auto;background:white">
    ...
    <div style="padding:24px">
      ...
      <div class="section-card">
        ...
        <div style="padding:14px">
          <div class="info-grid">
            ...
            <div style="grid-column:1/-1"><div class="info-label">Address</div>
              <div class="info-value">${[patient.address, patient.city, patient.state].filter(Boolean).join(', ') || '—'}</div></div>
          </div>
        </div>
      </div>
        ...
        <div style="padding:14px">
          <div style="margin-bottom:10px">
            ...
            <div style="margin-top:4px">
              ${patient.allergies
                ? patient.allergies.split(',').map(a => `<span class="allergy-badge">${a.trim()}</span>`).join(' ')
                : '<span style="color:#94A3B8">None recorded</span>'}
            </div>
          </div>
          <div>
            <div class="info-label">Chronic Conditions</div>
            <div class="info-value" style="margin-top:4px">${patient.chronic_conditions || 'None recorded'}</div>
          </div>
        </div>
      </div>
      ...
      <div class="section-card">
        ...
        <div style="padding:14px">
          <div class="info-grid">
            ...
            <div><div class="info-label">Insurance Provider</div><div class="info-value">${patient.insurance_provider}</div></div>
            <div><div class="info-label">Policy Number</div><div class="info-value">${patient.insurance_policy_number || '—'}</div></div>` : ''}
            ${patient.emergency_contact_name ? `
            <div><div class="info-label">Emergency Contact</div><div class="info-value">${patient.emergency_contact_name}</div></div>
            <div><div class="info-label">Emergency Phone</div><div class="info-value">${patient.emergency_contact_phone || '—'}</div></div>` : ''}
          </div>
        </div>
      </div>` : ''}
      ...
      <table>
        ...
        <tbody>${rowsOrEmpty(
          ...
          a => `<tr>
            ...
            <td><span class="badge ${a.status === 'Completed' ? 'badge-paid' : a.status === 'Cancelled' ? 'badge-due' : 'badge-issued'}">${a.status}</span></td>
          </tr>`,
      ...
      <table>
        ...
        <tbody>${prescriptions.slice(0,10).map(p => `<tr>
          <td>${p.generic_name}${p.brand_name ? ` (${p.brand_name})` : ''}</td>
          <td>${p.prescribed_dosage || '—'}</td>
          <td>${p.frequency || '—'}</td>
          <td><span class="badge ${p.status === 'Active' ? 'badge-paid' : 'badge-issued'}">${p.status}</span></td>
        </tr>`).join('')}</tbody>
      </table>` : ''}
...
<body>
  <div style="max-width:680px;margin:0 auto;background:white">
    <div class="header-logo" style="border-radius:8px 8px 0 0">
      <div>
        <div style="font-size:18px;font-weight:700">${CLINIC_NAME}</div>
        <div style="font-size:10px;opacity:0.8">Laboratory Result Report</div>
      </div>
      <div style="text-align:right;font-size:10px;opacity:0.8">
        <div>Order #${pad(order.order_id)}</div>
        <div>${new Date().toLocaleDateString('en-NG')}</div>
      </div>
    </div>
    ...
    <div style="padding:24px">
      ...
      <div class="info-grid" style="margin-bottom:20px">
        ...
        <div>
          ...
          <div style="margin-top:6px">
            <span style="background:${order.priority === 'Stat' ? '#FEE2E2' : order.priority === 'Urgent' ? '#FEF3C7' : '#F1F5F9'};
              color:${order.priority === 'Stat' ? '#991B1B' : order.priority === 'Urgent' ? '#92400E' : '#475569'};
      ...
      <div style="background:#F8FAFC;border-radius:6px;padding:12px;font-size:11px;color:#475569;margin-bottom:16px">${order.interpretation}</div>` : ''}
      ` : `
      ...
      <div class="info-grid">
        ...
        <div>
          ...
          <div class="info-value">${fmtDate(order.order_date)}</div>
        </div>
      </div>

      ${order.notes ? `
      <div class="section-title" style="margin-top:16px">Notes</div>
      <div style="background:#F8FAFC;border-radius:6px;padding:12px;font-size:11px;color:#475569">${order.notes}</div>` : ''}
